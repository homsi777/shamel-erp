...
(async()=>{
  ...
  const trialCard = page.locator('text=ميزان المراجعة العام').first();
  ...
  for(const w of widths){
    ...
    const m=await page.evaluate(()=>{
      ...
      return {
        ...
        hasReportRows:t.includes('مدين')||t.includes('دائن')||t.includes('الرصيد')
      }
    });
    const shot=path.resolve('reports/runtime-validation-shots',`reports-trial-output-${w}.png`);
