...
interface OpeningEntryHeaderProps {
  fiscalYear: string;
  entryDate: string;
  entryNumber: string;
  description: string;
  onChange: (next: Partial<OpeningEntryHeaderProps>) => void;
}

const OpeningEntryHeader: React.FC<OpeningEntryHeaderProps> = ({
  fiscalYear,
  entryDate,
  entryNumber,
  description,
  ...
  return (
    <div className="bg-white rounded-2xl border shadow-sm p-4 md:p-6">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-black text-gray-800">بيانات رأس القيد</h2>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div>
          <label className="block text-xs font-bold text-gray-500 mb-2">تاريخ القيد</label>
          <input
            type="date"
            value={entryDate}
        ...
        <div>
          ...
          <input
            ...
            placeholder="قيد أرصدة افتتاحية"
            className="w-full border rounded-xl p-2 font-bold text-gray-800"
          />
        </div>
      </div>
...
export default OpeningEntryHeader;
